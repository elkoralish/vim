...
" Chris changes =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
syn keyword pythonSpecial     True False Ellipsis None NotImplemented self
syn region  pythonClass       start="\s*self" end="\s*"
"syn region  pythonClass       start="\s*self" end="\s*[,\.]"
"syn region  pythonClass       start="\s*(*self" end="\s*[,\.]"
" =-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=
...
syn region pythonString       start=+'+ skip=+\\\\\|\\'\|\\$+ excludenl end=+'+ end=+$+ keepend contains=pythonEscape,pythonEscapeError,pythonFormat,@Spell
syn region pythonString       start=+"+ skip=+\\\\\|\\"\|\\$+ excludenl end=+"+ end=+$+ keepend contains=pythonEscape,pythonEscapeError,pythonFormat,@Spell
syn region pythonString       start=+"""+ end=+"""+ keepend contains=pythonEscape,pythonEscapeError,pythonDocTest2,pythonSpaceError,pythonFormat,@Spell
syn region pythonString       start=+'''+ end=+'''+ keepend contains=pythonEscape,pythonEscapeError,pythonDocTest,pythonSpaceError,pythonFormat,@Spell

syn region pythonDocstring  start=+^\s*[uU]\?[rR]\?"""+ end=+"""+ keepend excludenl contains=pythonEscape,pythonFormat,@Spell,pythonDoctest,pythonDocTest2,pythonSpaceError
syn region pythonDocstring  start=+^\s*[uU]\?[rR]\?'''+ end=+'''+ keepend excludenl contains=pythonEscape,pythonFormat,@Spell,pythonDoctest,pythonDocTest2,pythonSpaceError
...
" this one works
syn match  pythonFormat       +%[sxd]+ display contained
" this isn't working, and they both need to be added to the unicode strings below
syn match  pythonFormat       +{(.*?)}+  display contained
"syn match  pythonFormat       {([a-aA-Z0-9]+?)}  display contained
"syn match  pythonFormat       +\{[a-aA-Z0-9]+\}+ display contained
...
" Chris changes =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  HiLink pythonSpecial   Constant
  HiLink pythonClass    Constant
  "HiLink pythonOperator       Function
  HiLink pythonAssignment     Function
  HiLink pythonComparison     Function
  HiLink pythonArithmetic   Function
" =-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=
